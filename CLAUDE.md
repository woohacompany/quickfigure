...
## 핵심 성장 원칙 (모든 기능 구현 시 최우선 고려)
1. **구글 SEO 상위 노출** → 오가닉 트래픽 확보 (SEO 메타태그, JSON-LD 구조화 데이터, OG태그 필수)
2. **사용자 자발적 공유/임베드** → 백링크 자동 생성 (공유 버튼, 임베드 코드, 결과 공유 URL 등)
3. **모든 기능의 목표** = "더 많은 사람이 방문하고, 더 많은 외부 사이트에서 링크가 걸리는 구조"
